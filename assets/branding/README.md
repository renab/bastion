...
- `logos/svg/naffin-emblem.svg` — pure-vector emblem for sidebar navigation, compact headers, and scalable UI placement.
- `logos/svg/naffin-lockup.svg` — pure-vector full lockup for headers, documentation, and print-like uses where a self-contained vector is required. All lettering is path geometry; no local font is needed.
- `logos/svg/naffin-lockup-hybrid.svg` — preferred full-fidelity lockup for digital headers and splash treatments. The compass, orbit, and lettering remain vector while the luminous wormhole uses the separately referenced raster core.
...
| Vector-only delivery | `naffin-emblem.svg` or `naffin-lockup.svg` |
...
- The hybrid SVG intentionally combines vector geometry with `wormhole-core-2048.png` to preserve the turbulent, luminous appearance of the approved reference. Keep their relative directory structure intact so the reference resolves.
